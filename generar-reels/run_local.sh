#!/usr/bin/env bash
# Local end-to-end pipeline: insight → audio → html → render → publish IG → notify Discord
# Run from generar-reels/ directory.
#
# Flags:
...
if [[ "$USE_EXAMPLE" == "true" ]]; then
  cp examples/insight_example_mtc.json "$INSIGHT_PATH"
